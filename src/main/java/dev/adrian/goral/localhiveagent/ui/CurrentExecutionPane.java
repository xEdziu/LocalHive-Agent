...
class CurrentExecutionPane extends VBox {

    private final Label titleLabel;
    private final StatusBadge statusBadge;
    private final Label executorLabel;
    private final Label technicalExecutorLabel;
    private final Label executionIdLabel;
    private final Label durationLabel;
    private final Label workspaceLabel;
    private final Label lastErrorLabel;

    CurrentExecutionPane() {
        this.titleLabel = new Label("No execution running");
        ...
        this.executionIdLabel = new Label();
        this.durationLabel = new Label();
        this.workspaceLabel = new Label();
        this.lastErrorLabel = new Label();
        ...
        addRow(grid, 0, Feather.CPU, "Executor", executorLabel);
        addRow(grid, 1, Feather.FILE_TEXT, "Technical ID", technicalExecutorLabel);
        addRow(grid, 2, Feather.HASH, "Execution ID", executionIdLabel);
        addRow(grid, 3, Feather.CLOCK, "Duration", durationLabel);
        addRow(grid, 4, Feather.FOLDER, "Workspace", workspaceLabel);
        addRow(grid, 5, Feather.ALERT_CIRCLE, "Last error", lastErrorLabel);

        getChildren().addAll(header, grid);
        refresh(CurrentExecutionViewModel.empty());
    }

    void refresh(CurrentExecutionViewModel viewModel) {
        ...
        executionIdLabel.setText(viewModel.executionId());
        durationLabel.setText(viewModel.duration());
        workspaceLabel.setText(viewModel.workspace());
        lastErrorLabel.setText(viewModel.lastError());
    }
